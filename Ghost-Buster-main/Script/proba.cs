...
class Game {
		...
		public static Dictionary<color, double> R_Dict, O_Dict, Y_Dict, G_Dict;
		R_Dict = new Dictionary<color, double>() {
		{ color.R, 0.87 }, { color.O, 0.10 }, { color.Y, 0.02 }, { color.G, 0.01 },};
		O_Dict = new Dictionary<color, double>() {
		{ color.R, 0.08 }, { color.O, 0.88 }, { color.Y, 0.03 }, { color.G, 0.01 },};
		Y_Dict = new Dictionary<color, double>() {
		{ color.R, 0.01 }, { color.O, 0.04 }, { color.Y, 0.86 }, { color.G, 0.08 },};
		G_Dict = new Dictionary<color, double>() {
		{ color.R, 0.04 }, { color.O, 0.06 }, { color.Y, 0.10 }, { color.G, 0.80 },};            
		
	}
	
	public class initiate_game {
		public static void init(Tile> tiles, int w, int height, Dictionary<Vector2) {
			this.tiles = tiles;
			this.w = w;
			l = height;
	    ...
	    public static void init_proba() {
	        ...
	        for (int x = 0; x < w; x++) {for (int y = 0; y < l; y++) probabilities[i,j] = init_proba;}
	    }
